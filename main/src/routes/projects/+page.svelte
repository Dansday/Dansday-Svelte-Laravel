...
<main class="grid flex-1 grow gap-2 overflow-y-auto md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
	{#each data.items as project, i (project.id)}
		{@const posterUrl = resolveImageUrl(project.poster, baseUrl)}
		<a
		...
		>
			<figure class="group bg-ash-700 relative aspect-video overflow-hidden">
				{#if posterUrl}
					<img
						src={posterUrl}
						alt={project.title}
				...
				{/if}
				<div
					class="absolute top-0 left-0 grid h-full w-full place-items-center bg-[#080808]/90 transition-opacity duration-500 {posterUrl
